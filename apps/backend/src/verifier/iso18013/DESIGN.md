...
| 7 | **Resolved** | ~~`readerAuth` (verifier authentication inside the `DeviceRequest`) is not implemented~~ | — | Opt-in per presentation config via `readerAuth: true`. When enabled, the offer signs `ReaderAuthentication = ["ReaderAuthentication", SessionTranscript, ItemsRequestBytes]` as a detached COSE_Sign1 with the tenant's Access key chain (`buildReaderAuth` in `cbor-request.ts`) and embeds it in the `DocRequest`. The DCAPIHandover transcript is computed at offer time from the `encryptionInfo` + browser origin. Byte-exactness is verified by a sign→`ReaderAuth.verify()` round-trip test (`reader-auth.spec.ts`). Note: signing extracts the Access private key as a JWK, so KMS-backed non-extractable keys are not yet supported |
